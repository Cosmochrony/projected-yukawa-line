...
\title{
  ...
  Functorial Closure of $\LY = \wedge^{2}(\Sbundle)$ and Spectral Ordering on $\Cgen$
}
...
\begin{document}
  ...
  \begin{abstract}
    ...
    metaplectic Weil carrier~\cite{Beau2026q14}, the determinant line $\LY := \wedge^{2}(\Sbundle)$ is the unique
    ...
    residue takes the closed normal form $\Epi^{2}|_{\Cgen} = \diag(1, \tfrac12 + u, \tfrac12 - u)$~\cite{Beau2026prs},
    whose exit deficits $\{0, \tfrac12 - u, \tfrac12 + u\}$ order the three generation levels~\cite{Beau2026q14}.
  ...
  \section{Generation-level assignment}
    ...
    The CP-even real sector imposes $v = 0$, and the even part is closed analytically by Born--Infeld parity on the
    equatorial admissible fibre, $(C_{2} - \Jthree^{2})/C_{2} = \diag(1, \tfrac12, \tfrac12)$~\cite{Beau2026a34}.
    ...
    \begin{proof}
      The normal form is~\cite{Beau2026prs} with $v = 0$ and the even closure~\cite{Beau2026a34}; its eigenvalues on
  ...
  \section{Status of results}
    ...
    \paragraph{Derived (this note).}
      ...
      Both are structural and unconditional given the carrier identifications of~\cite{Beau2026q14, Beau2026prs,
      Beau2026a34}.
  ...
  \section{Reproducibility}
    ...
    floating-point sampling, in \texttt{simulation/fermionic-matter/front3\_yukawa\_line.py}.
